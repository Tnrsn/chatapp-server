...
public interface FriendshipRepository extends JpaRepository<Friendship, UUID> {
	...
	@Query("""
	    DELETE FROM Friendship f
	    WHERE (f.requesterId = :userId AND f.receiverId = :otherId)
	       OR (f.requesterId = :otherId AND f.receiverId = :userId)
	""")
	void deleteFriendship(@Param("userId") UUID userId, @Param("otherId") UUID otherId);
}
